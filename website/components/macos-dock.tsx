...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const getResponsiveConfig = useCallback(() => {
    if (typeof window === "undefined") {
      return { baseIconSize: 48, maxScale: 1.5, effectWidth: 180 }
    }
    ...
    if (smallerDimension < 480) {
      // Mobile phones
      return {
        baseIconSize: Math.max(32, smallerDimension * 0.07),
        maxScale: 1.3,
      ...
      // Small laptops
      return {
        baseIconSize: Math.max(42, smallerDimension * 0.045),
        maxScale: 1.5,
      ...
      return {
        baseIconSize: Math.max(48, Math.min(56, smallerDimension * 0.035)),
        maxScale: 1.6,
        effectWidth: 220,
      }
    }
  ...
  // Start with SSR-safe fixed default, calculate correct value on mount
  const [config, setConfig] = useState({ baseIconSize: 48, maxScale: 1.5, effectWidth: 180 })
  const { baseIconSize, maxScale, effectWidth } = config
  const minScale = 1.0
  const baseSpacing = Math.max(4, baseIconSize * 0.08)

  // Calculate responsive config on mount and update on window resize
  useEffect(() => {
    const updateConfig = () => {
  ...
  const animateToTarget = useCallback(() => {
    const targetScales = calculateTargetMagnification(mouseXRef.current)
    const targetPositions = calculatePositions(targetScales)
    const lerpFactor = mouseXRef.current !== null ? 0.2 : 0.12
    ...
    if (scalesNeedUpdate || positionsNeedUpdate || mouseXRef.current !== null) {
          ...
          return (
            ...
            >
              <Image
                ...
                style={{
                  transform: `scale(${app.iconScale ?? 1})`,
